...
module private Helpers = 
    let newValues = new Dictionary< Key, Value list >()
...
let getSavedValues k = 
    let fn = Repository.varPath (false,k.Party,k.Product,k.Var)     
...
let save() = lock newValues <| fun () ->
    ...
    let files = new Dictionary<Key, BinaryWriter>()      
    let getWriter k =            
        match files.TryGetValue(k) with
        | true, x -> x
        | _ ->  
            let path = Repository.varPath(true, k.Party, k.Product, k.Var)
...
let delete (partyPath,productId) = lock newValues <| fun () ->
    let folderPath = Repository.productFolderPath (false,partyPath,productId)
