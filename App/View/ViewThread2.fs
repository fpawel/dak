...
module Delay = 
    ...
    let initialize =        
        ...
        Dak.Operations.Delay.onUpdate.Value <- fun start'time get'time -> 
...
let initialize = 
    ...
    Thread2.addKeepRunningChanged <| function
        ...
        | _ -> ()
    Dak.Operations.Scenaries.ModalMessage.onShow.Value <- fun title level text ->            
    ...
    Dak.Operations.Scenaries.ModalMessage.onClose.Value <- fun () ->            
        form.PerformThreadSafeAction <| fun () -> 
            panelModalMessage.Visible <- false

    Dak.Operations.Scenaries.ModalMessage.getIsVivisble.Value <- fun () ->
    ...
    Thread2.addKeepRunningChanged <| fun (_,keep'running) ->
        form.PerformThreadSafeAction <| fun () ->
