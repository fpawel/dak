...
module private Helpers =
    ...
    let binding<'a when 'a : equality> (t:TextBox) textToValue valueToText eq subj prop scalingMeth =    
        ...
        and onTextChanged = EventHandler(onTextChangedFun)
    ...
    let asvm = Dak.Chart.axisScalingViewModel
...
module private Helpers1 =
    ...
    let comboBoxVars = 
        ...
        let update _ =       
            let selectedItem = x.SelectedItem :?> string
            Dak.Chart.physVar <- 
                List.find
                    (fun x -> Dak.MilVar.name x = selectedItem ) 
                    Dak.MilVar.values
