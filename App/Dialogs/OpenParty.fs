...
module private Helpers =
    ...
    let updateTreeView (treeview:TreeView) (nodes:ResizeArray<Node * TreeNode>) prodType serial month year = 
        ...
        AppData.getParties prodType serial month year
            ...
            xs |> Seq.iter( fun (month,xs) -> 
                ...
                xs |> Seq.iter( fun (day,xs) ->                         
                    ...
                    xs |> Seq.iter( fun b -> 
                        ...
                        if b.ID = party.Party.PartyInfo.ID  then
                            selectedNode <- node
    ...
    let createDialog () = 
        ...
        let acceptFilter _ =
            ...
            let serial = 
                String.tryParseDecimal tbSerialNumber.Text
            let month = 
        ...
        buttonAccept.Click.Add <| fun _ ->
            ...
            | Some b -> 
                ...
                | Some error -> 
                    let message = 
                        sprintf 
                            "Не удалось открыть данные партии %A, %A. %s" 
